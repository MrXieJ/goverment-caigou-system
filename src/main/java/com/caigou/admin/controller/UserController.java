...
public class UserController {
     ...
     * 用户姓名，用户账号，用户角色类型
     * */
    @RequestMapping("/find/bycondition")
    public Map getUser(User user){
        Long limit = -1l;
        Long first = -1l;
        if (user.getLimit() != null) {
            limit = Long.valueOf(user.getLimit());
            first = (user.getPage() - 1) *limit;
        }
        user.setLimit(limit);
        user.setPage(first);
        Map map = new HashMap();
    ...
    public Map addUser(User user){
        Map map=new HashMap();
        user.setLast_update(DateUtils.getNowFormatDate(new Date()));
        try {
